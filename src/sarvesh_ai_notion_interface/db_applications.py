...
from .config import NOTION_API, NOTION_DB_APPLICATIONS, NOTION_TOKEN
...
def parse_inline(text):
    ...
    for match in pattern.finditer(text):
        ...
        if match.group(2):
                ...
                rich_text.append(
                    {
                        "type": "text",
                        "text": {
                            "content": link_in_bold.group(1),
                            "link": {"url": link_in_bold.group(2)},
                        },
            ...
            rich_text.append(
                {"type": "text", "text": {"content": match.group(6)}, "annotations": {"code": True}}
            )
        elif match.group(8):
            rich_text.append(
                {
            ...
            blocks.append(
                {
                    ...
                    "numbered_list_item": {"rich_text": parse_inline(num_match.group(1))},
                }
            )
            i += 1
            continue
    ...
    if resume_upload_id:
        properties["Resume"] = {
            "files": [
                {
                    "type": "file_upload",
                    "file_upload": {"id": resume_upload_id},
...
def main():
    ...
    if args.command == "add":
        ...
        result = update_evaluation(
            args.page_id, args.score, args.status, args.report_file, args.resume_file
        )
        print(json.dumps(result, indent=2))
