"""Configuration for Notion-backed job-tracking workflows.

Values are read from environment variables. A local ``.env`` file in the
current working directory is an optional development convenience; callers may
instead point ``AI_SKILLS_ENV_FILE`` at one specific file. Existing environment
...
def _candidate_env_files() -> list[str]:
    """Return explicit and local development .env paths, in precedence order."""
    candidates: list[str] = []
    override = os.environ.get("AI_SKILLS_ENV_FILE")
    if override:
...
def _load_env() -> None:
    """Load variables from the first configured .env file into ``os.environ``.
